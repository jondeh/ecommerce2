...
module.exports = {
  ...
  getMosquitoData: async (req, res) => {
    ...
    try {
      let locationKey = await axios
        .get(
          `https://dataservice.accuweather.com/locations/v1/cities/geoposition/search?apikey=${ACCU_API}&q=${lat}%2C${lng}&language=en-us&details=true&toplevel=false`
        )
        .then(res => res.data)
      ...
      let mosquitoData = await axios
        .get(
          `https://dataservice.accuweather.com//indices/v1/daily/5day/${locationKey.Key}/17?apikey=${ACCU_API}&language=en-us&details=false`
        )
        .then(res => res.data)
  ...
  getOpenWeather: async (req, res) => {
    ...
    try {
      ...
      let openWeather = await axios
        .get(
          `https://api.openweathermap.org/data/2.5/onecall?lat=${lat}&lon=${lng}&units=imperial&appid=${OPEN_WEATHER_API}`
        )
        .then(res => res.data)
  ...
  getDayWeather: async (req, res) => {
    ...
    try {
      ...
      let dayWeather = await axios
        .get(
          `https://dataservice.accuweather.com/currentconditions/v1/${locationKey.Key}?apikey=${ACCU_API}`
        )
        .then(res => res.data)
